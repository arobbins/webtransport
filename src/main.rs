...
impl Config {
    fn from_env() -> Result<Self> {
        ...
        Ok(Self { ticker_interval, log_level, cert_hash, server_config })
    }
}
...
fn spawn_ticker(connection_list: ConnectionList, ticker_interval: u64) {
    tokio::spawn({
        async move {
...
async fn start_server(config: Config) -> Result<()> {
    ...
    spawn_ticker(connection_list.clone(), config.ticker_interval);
